...
export function GET() {
  ...
  const lines: string[] = [
    ...
    `> ${site.description}`,
    "",
    `${site.name}, ${site.city}'un Adalar hariç tüm ilçelerinde 7/24 talep üzerine (on-demand) kurye hizmeti verir. Ayrıca ${site.nearbyDistricts.join(", ")} gibi çevre ilçelere ve ${site.provinces.join(", ")} illerine de gönderi taşır. Ana hizmetler: moto kurye, acil kurye, aynı gün teslimat, paket & koli kurye, kurumsal kurye ve e-ticaret teslimat.`,
    "",
    "## Hizmetler",
    ...
    `- E-posta: ${site.email}`,
    `- Hizmet bölgeleri: ${site.city} (${site.istanbulNote})`,
    `- Çevre ilçeler: ${site.nearbyDistricts.join(", ")}`,
    `- Çevre iller: ${site.provinces.join(", ")}`,
